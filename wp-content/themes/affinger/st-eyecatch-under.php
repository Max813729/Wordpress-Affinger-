...
if ( st_has_post_thumbnail() && (( isset($GLOBALS['stdata53']) && $GLOBALS['stdata53'] === 'yes' ) || ( isset( $eyecatchset ) && $eyecatchset === 'yes' ))) { ?>
	<div class="<?php st_eyecatch_class(); ?> <?php if(trim($GLOBALS['stdata217']) !== ''){ echo 'st-eyecatch-under'; } //アイキャッチ画像がタイトル下の場合 ?>">
		...
		if ( $youtube_thum_img ) {
			if ( ! $st_youtube_eyecatch && isset($GLOBALS['stdata241']) && $GLOBALS['stdata241'] === 'photo' ) { // ポラロイドデザイン ?>
				<p id="caption-attachment" class="wp-caption-text st-youtube-copyurl"><i class="st-fa st-svg-youtube-play" aria-hidden="true"></i><a href="<?php echo esc_url( $youtube_link ); ?>" target="_blank" rel="nofollow">YouTube</a></p>
			<?php } else {
				if( ! $st_youtube_eyecatch ) { // 動画の場合はクレジット無し ?>
			...
			if ( isset($GLOBALS['stdata241']) && $GLOBALS['stdata241'] === 'photo' ) { // ポラロイドデザイン ?>
				<p id="caption-attachment" class="wp-caption-text"><i class="st-fa st-svg-copyright" aria-hidden="true"></i><?php echo esc_url( get_home_url()); ?></p>
			<?php } else { ?>
			...
			if ( isset($GLOBALS['stdata241']) && $GLOBALS['stdata241'] === 'photo' ) { // ポラロイドデザイン ?>
				<p id="caption-attachment" class="wp-caption-text"><i class="st-fa st-svg-camera-retro" aria-hidden="true"></i><?php echo stripslashes( $stcopyurl ); ?></p>
			<?php } else { ?>
			...
			if ( isset($GLOBALS['stdata241']) && $GLOBALS['stdata241'] === 'photo' ) { // ポラロイドデザイン ?>
				<p id="caption-attachment" class="wp-caption-text"><i class="st-fa st-svg-camera-retro" aria-hidden="true"></i><?php echo get_post(get_post_thumbnail_id())->post_excerpt;?></p>
			<?php } else { ?>
