...
$is_thumbnail_available = st_has_term_thumbnail();
?>
<div id="content" class="clearfix">
    <div id="contentInner">
        <main>
            <article>
				<?php if ( ! $is_thumbnal_under && $is_thumbnail_available ): // サムネイル ?>
      ...
      itemtype="http://schema.org/ListItem"><a href="<?php echo esc_url( get_category_link( $catid ) ); ?>" itemprop="item">
					...
					<!--/ ぱんくず -->
        <?php
		$now_category = get_query_var('cat'); //現在のカテゴリーIDを取得
		$args = array(
		...
		if(trim($cat_data['listdelete']) === ''){
			...
			<?php if ( is_active_sidebar( 21 ) ) { ?>
				<?php if ( function_exists( 'dynamic_sidebar' ) && dynamic_sidebar( 21 ) ) : else : //カテゴリーページ上一括ウィジェット ?>
				<?php endif; ?>
			<?php } ?>
		...
		<?php } ?>
			...
			<?php if ( is_active_sidebar( 22 ) ) { ?>
				<?php if ( function_exists( 'dynamic_sidebar' ) && dynamic_sidebar( 22 ) ) : else : //カテゴリーページ下一括ウィジェット ?>
				<?php endif; ?>
			<?php } ?>
